...
def analyze_dataset_en(filename):
    ...
    # small_df.to_csv('/mount/arbeitsdaten/studenten2/caulfiea/masters_thesis/en_other_occ_first_sentence.csv')
...
def analyze_dataset(filename, gender):
    ...
    full_df = pd.DataFrame(list(full_unique_occupations), columns=['occupations'], index = None)
    # full_df.to_csv('/mount/arbeitsdaten/studenten2/caulfiea/masters_thesis/de_occ_list_unique.csv', index=None)
    ...
    print("length of dataset:")
    print(small_df.shape[0])
    print("intros where female tokens are found in male intros:")
    print(smaller_df.shape[0])
    # smaller_df.to_csv('/mount/arbeitsdaten/studenten2/caulfiea/masters_thesis/de_female_no_male_last_name.csv')
...
# analyze_dataset_en("/mount/arbeitsdaten/studenten2/caulfiea/masters_thesis/datasets/en_other_dataset.csv")
...
# full_unique_occupations = get_occupations_list('/mount/arbeitsdaten/studenten2/caulfiea/masters_thesis/de_occ_titles.csv', 'male')
# full_unique_occupations = list(full_unique_occupations)
# print(len(full_unique_occupations))
# # full_unique_occupations = [re.sub(r'[“”‘’"]', '', s.strip()) for s in full_unique_occupations]
# print(len(full_unique_occupations))
# full_unique_occupations = [s.strip('"') for s in full_unique_occupations]
# filtered_occupations = [s for s in full_unique_occupations if not s.endswith('"')]
# filtered_occupations = [s for s in full_unique_occupations if not s.endswith("-")]
# print(len(filtered_occupations))
# filtered_occupations.sort()
# new_df = pd.DataFrame(filtered_occupations, columns=['occupations'], index = None)
# new_df.to_csv('/mount/arbeitsdaten/studenten2/caulfiea/masters_thesis/demale_occ_list_unique.csv', index=None)
...
def remove_html_en(filename):
    ...
    print(small_df['intro'].head(10))
    # small_df.to_csv('/mount/arbeitsdaten/studenten2/caulfiea/masters_thesis/en_other_nohtml.csv', index=None, header = None)

# remove_html_en("/mount/arbeitsdaten/studenten2/caulfiea/masters_thesis/datasets/en_other_dataset.csv")