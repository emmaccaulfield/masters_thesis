...
def get_named_entities(text):
  ...
  ner = spacy.load('it_core_news_sm',disable = ['tagger', 'parser']) 
  # ner = spacy.load('es_core_news_sm', disable = ['tagger', 'parser'])
  doc = ner(text) 
...
def get_occupations(df):
    ...
    def find_terms_stems(text):
      ...
      # remove accents/diacritics from text for spanish
      # text_decoded = unidecode(text)
      # ner = spacy.load('es_core_news_sm', disable = ['parser'])
      # doc = ner(text_decoded) 
      doc = ner(text)
      occupational_titles = list(full_unique_occupations)
      occupational_titles.append("tore")
      occupational_titles.append("trice")
      # occupational_titles.append('política')
      # occupational_titles.append('político')
      matched_occupations = []
      for ent in doc: 
    ...
    small_df.to_csv('/mount/arbeitsdaten/studenten2/caulfiea/masters_thesis/sets_for_analysis/redo/it_other_occ.csv')

it_female_df = remove_html_en('/mount/arbeitsdaten/studenten2/caulfiea/masters_thesis/datasets/redo/it_other_dataset.csv')
